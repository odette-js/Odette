var COLLECTION = 'Collection',
    ...
    Collection = app.block(function (app) {
        var isNullMessage = 'object must not be null or ' + UNDEFINED,
            ...
            recreatingSelfCollection = toArray('eq,where,whereNot,map,results,filter,cycle,uncycle,flatten,gather,unique').concat(keys(buildCallers('map'), keys(buildCallers('mapRight')))),
            eachHandlers = {
                each: duff,
                duff: duff,
                forEach: duff,
