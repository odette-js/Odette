...
app.scope(function (app) {
    var _ = app._,
        ...
        executeHandlers = function (deferred, obj) {
            var handler, catches = [],
                ...
                // catchesCanRun, arg = deferred[STASHED_ARGUMENT] || deferred[REASON],
                handlers = deferred[STASHED_HANDLERS],
        ...
        /**
         * A class for creating deferreds that execute synchonously whenever possible. Deferreds have a long and complex history. While it is generally accepted that deferreds will operate the way they do now under the [Deferreds]{@see mdn} documentation on mdn. While it is best to use the callback, then, catch pattern available for deferreds, it may also be necessary to resolve deferreds externally, preventing extra logic from being needed, but allowing for logic to be customized whenever needed.
         * @example <caption>To reiterate: It is best to use the callback, then, catch pattern as defined below. Note the only difference is the <code>_.Deferred</code> piece. This is so we can provide extra methods to our users.</caption>
         * return _.Deferred(function (success, failure) {
         *     // runs async code
         * }).then(function () {
         *     // deferred and internal deferreds are complete
         * }).catch(function () {
         *     // deferred or internal deferreds failed
         * });
         * @example <caption>if you do not want to have the leading underscore then simply use the following code to set a <code>Deferred</code> variable and to back it up with the window.</caption>
         * var Deferred = (_ && _.Deferred) || function (deferred) {
         *     return deferred instanceof window.Deferred ? deferred : return new window.Deferred(deferred);
         * };
         ...
         *     // just like the all / race members on the Deferred constructor
         * });
         * @class Deferred
         */
        Deferred = factories[DEFERRED] = _[DEFERRED] = Events.extend(DEFERRED,
             ...
             */
            {
                ...
                constructor: function (handler, bool) {
                    ...
                    deferred[STASHED_HANDLERS] = [];
                    // deferred[REASON] = BOOLEAN_FALSE;
                    ...
                    // var call = function () {
                    //     handler(bind(deferred.fulfill, deferred), bind(deferred.reject, deferred), bind(deferred.resolveAs, deferred));
                    // };
                    // if (!isFunction(handler)) {
                    //     console.error("Uncaught TypeError: Deferred resolver undefined is not a function");
                    //     erred = BOOLEAN_TRUE;
                    // }
                    // if (bool) {
                    //     deferred.mark('synchonous');
                    // }
                    // call();
                    // if (!erred) {
                    //     if (bool) {
                ...
                stash: intendedApi(function (name, list) {
                    var deferred = this,
                        stashedHandlers = deferred[STASHED_HANDLERS];
        ...
        emptyQueue = function (p, bool, arg) {
            ...
            sliced.each(function (current) {
                ...
                if (isPromise(argument)) {
                    argument.then(function (res) {
                        return emptyQueue(nextp, BOOLEAN_TRUE, res);
                    }, function (res) {
                        return emptyQueue(nextp, BOOLEAN_FALSE, res);
                    });
                } else {
        ...
        Promise = _[PROMISE] = factories.Directive.extend(PROMISE, {
            ...
            then: function (whensuccessful, whenfailed) {
                var success, failure, promise = this;
            ...
            catch: function (whenfailed) {
                var promise = this;
                return promiseProxy(function (pro, success, failure) {
                    if (promise.is(CAUGHT)) {
                        success(whenfailed(resultant(promise)));
                    } else {
                        addToQueue(promise, CATCHING, [whenfailed, promise]);
                    }
                });
            }
        }),
        DeferredPrototype = Deferred[CONSTRUCTOR][PROTOTYPE],
        resulting = DeferredPrototype.addMethod(SUCCESS).addMethod(FAILURE).addMethod(ALWAYS).addMethod(CATCH),
    ...
    duff(toArray('all,race'), function (key) {
        Deferred[key] = function () {
            var deferred = Deferred(noop);
            return deferred[key].apply(deferred, arguments);
        };
    });
    app.extend({
        dependency: bind(Deferred.all)
    });
    _.publicize({
